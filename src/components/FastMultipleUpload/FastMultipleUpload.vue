...
async function handleBeforeUpload(options: { file: UploadFileInfo; fileList: UploadFileInfo[] }) {
  if (props.data.length >= props.maxSize) {
    window.$message.error("图片数量超出限制")
    return false
  }
...
<template>
  <n-upload @before-upload="handleBeforeUpload" :max="props.maxSize" multiple>
    <slot />
  </n-upload>
</template>
